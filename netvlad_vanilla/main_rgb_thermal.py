...
parser.add_argument('--cachePath', type=str, default='./NetVLAD/cache', help='Path to save cache to.')
...
def test(eval_set, epoch=0, write_tboard=False):
    ...
    for qIx, pred in enumerate(predictions):
        for i,n in enumerate(n_values):
            ...
            if np.any(compare):
                correct_at_n[i:] += 1
                idx = np.where(compare == True)[0][0]
                if write_tboard: 
                    query_img = np.concatenate((Image.open(eval_set.dbStruct.qImage[qIx][0]),Image.open(eval_set.dbStruct.qImage[qIx][1])),axis=1)
                    writer.add_image(str(epoch)+'_query_img', ToTensor()(query_img),counter)
                    # print(predictions[qIx,idx])
                    q_img = np.concatenate((Image.open(eval_set.dbStruct.dbImage[predictions[qIx,idx]][0]),Image.open(eval_set.dbStruct.dbImage[predictions[qIx,idx]][1])),axis=1)
                    writer.add_image(str(epoch)+'_answer',ToTensor()(q_img),counter)
                    counter += 1
                break
...
if __name__ == "__main__":
    ...
    if opt.dataset.lower() == 'sthereo':
        if opt.RGB:
            ...
            import sthereo_thermal_multi_channel as dataset
    else:
