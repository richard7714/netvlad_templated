...
def input_transform():
    return transforms.Compose([
        ...
        transforms.Normalize(mean=[0.45,0.45],
                               std=[0.2,0.2]),
...
class WholeDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, input_transform=None, onlyDB=False):
        ...
        self.rgb_images = [join(root_dir, dbIm) for dbIm in self.dbStruct.dbImage[:,0] if isinstance(dbIm, str)]
        self.ir_images = [join(root_dir, dbIm) for dbIm in self.dbStruct.dbImage[:,1] if isinstance(dbIm, str)]
        if not onlyDB:
            self.rgb_images += [join(root_dir, qIm) for qIm in self.dbStruct.qImage[:,0] if isinstance(qIm, str)]
            self.ir_images += [join(root_dir, qIm) for qIm in self.dbStruct.qImage[:,1] if isinstance(qIm, str)]
    ...
    def __getitem__(self, index):
        rgb_img = np.array(Image.open(self.rgb_images[index]).convert('L'))
        ir_img = np.array(Image.open(self.ir_images[index]).convert('L'))
        
        img = np.array([rgb_img,ir_img])
...
def collate_fn(batch):
    ...
    negCounts = data.dataloader.default_collate([len(x) for x in negatives])
    
    # negatives => 2 X 10 X 10
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, nNegSample=1000, nNeg=5, margin=0.1, input_transform=None):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        vis_query = np.array(Image.open(join(root_dir, self.dbStruct.qImage[index][0])).convert('L'))
        ir_query = np.array(Image.open(join(root_dir, self.dbStruct.qImage[index][1])).convert('L'))
        vis_positive = np.array(Image.open(join(root_dir, self.dbStruct.dbImage[posIndex][0])).convert('L'))
        ir_positive = np.array(Image.open(join(root_dir, self.dbStruct.dbImage[posIndex][1])).convert('L'))

        query = np.array([vis_query,ir_query])

        query = np.transpose(query,(1,2,0))

        positive = np.array([vis_positive,ir_positive])
        ...
        for negIndex in negIndices:
            vis_negative = np.array(Image.open(join(root_dir, self.dbStruct.dbImage[negIndex][0])).convert('L'))
            ir_negative = np.array(Image.open(join(root_dir, self.dbStruct.dbImage[negIndex][1])).convert('L'))

            negative = np.array([vis_negative,ir_negative])
